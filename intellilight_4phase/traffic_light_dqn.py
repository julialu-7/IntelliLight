...
Main experiment runner for training a deep reinforcement learning traffic light agent.
- Initializes configuration, paths, agent
- Handles training and pretraining loops
- Interfaces with SUMO simulation through SumoAgent
"""
...
class TrafficLightDQN:
    ...
    class PathSet:
        """Handles loading paths and configs for an experiment run."""
        EXP_CONF = "exp.conf"
    ...
    def _generate_pre_train_ratios(self, phase_min_time, em_phase):
        ...
        for i, phase_time in enumerate(phase_min_time):
            """Generate different phase configurations for pretraining."""
            if i == em_phase:
    ...
    def _set_traffic_file(sumo_config_file_tmp_name, sumo_config_file_output_name, list_traffic_file_name):
        """Insert traffic file name(s) into SUMO config XML."""
    ...
    def set_traffic_file(self):
        """Configure traffic files and copy them to output directory."""
